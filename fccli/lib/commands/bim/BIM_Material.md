---
command: "BIM_Material"
generated:                     # owned by the tool; rewritten on reconcile
  freecad: "1.1.3"
  label: "Material"
  tooltip: "Sets or creates a material for selected objects"
  toolbar: "Manage Tools"
  menu: "Manage"
  shortcut: "M, A"
  workbench: "BIMWorkbench"
  wiki: "BIM_Material"
  wiki_rev: "0499378"
# authored from here down; the tool never rewrites these
verb: null
aliases: []
requires: []
panel: null
family: null
choice: null
rank: null
type: null
---

Shows the BIM material dialog. The dialog allows to quickly and easily perform material-related operations, with extra focus on efficient working with many objects and many materials.

1. Create new material or multimaterial 2. Attribute an existing material or multimaterial to selected object(s).
